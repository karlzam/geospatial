...
def ckdnearest(gdA, gdB):
    ...
    # Update this to kdtree bc this is an older version
    # KDTree enables quicker nearest neighbour search (for creates a tree for the tp's)
    btree = KDTree(nB)
    # dist and idx from nB (so idx from the tp)
...
if __name__ == "__main__":
    ...
    for idx, year in enumerate(years):
        ...
        #nbac_buff['geometry'] = nbac_buff.geometry.apply(lambda geom: compute_concave_hull(geom, alpha=1.5))
       ...
       # row2 = nbac_buff_concavh15.loc[index]
        ...
        #nbac_buff.to_file(shp_dir + '\\' + 'nbac-buff-concav' + str(doi_firms) + '.shp')
        ...
        nbac_buff = gpd.read_file(shp_dir + '\\' + 'nbac-buff-concav2023-09-24.shp')
        ...
        #nbac_buff_concavh15['perim-source'] = 'NBAC'
        ...
        #df_perims = pd.concat([nbac_buff_concavh15, nfdb_buff, pers_hs_cad_buff])
        ...
        # Updated to use the concave buffered
        # need to convert the epsg first before doing this, but it works
        if len(NBAC_agg) > 0:
            #nbac_4326 = nbac_buff_conc.to_crs("EPSG:4326")
            nbac_4326 = nbac_buff.to_crs("EPSG:4326")

            # ok so there's potentially multiple rows with the same NFIREID... ugh NBAC.
            # trying to make sure i'm getting the proper ID row, but it looks like the idx is off?
            # just went through the KDTree thing and I don't think it should be off...
            # let's program it like they're not off and then double check, maybe I'm missing something
            # want to pull the buffered perimeter using the ID, but NBAC has it listed multiple times.
            # I guess I'll just grab them all, combine the polygons, and plot that...
            # Is there a way to grab just the specific buffered polygon that the tp was included in?
            # I thought that would be the idx from the tp database, but that just stored the NBAC ID
            # Ya ok it makes the most sense to combine all of the rows with the same ID because of the way I'm
            # aggregating the the fp's
            # TODO: REMEMBER TO CHANGE BACK TO THE PROPER BUFFERED POLYGON AFTER, NOT READING IN FILES
            #NBAC_concatd_perims = concat_perims(NBAC_agg, nbac_4326)
            NBAC_agg['fire-perimeter'] = concat_perims(NBAC_agg, nbac_4326, 'NFIREID')

        if len(NFDB_agg) > 0:
            nfdb_4326 = nfdb_buff.to_crs("EPSG:4326")
            NFDB_agg['fire-perimeter'] = concat_perims(NFDB_agg, nfdb_4326, 'NFDBFIREID')
